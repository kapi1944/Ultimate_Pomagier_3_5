...
# Ultimate Pomagier — miniatury zadań z kadrowaniem — BEZ CODEXA / BEZ PYTHONA
# Uruchom w root repo:
...
function replaceOnce(text, oldText, newText, label) {
  ...
  if (first < 0) throw new Error(`${label}: nie znaleziono oczekiwanego fragmentu.`);
  const second = text.indexOf(oldText, first + oldText.length);
  if (second >= 0) throw new Error(`${label}: znaleziono więcej niż jedno dopasowanie. Patch przerwany.`);
...
// ---------------------------------------------------------------------------
{
  const path = 'src/moduly/zamkniete/pulpit/modele/pulpit.ts';
  let text = read(path);

  const oldText = `export type MiniaturaZadaniaPulpitu = {
  daneUrl: string
  nazwaPliku: string
  szerokosc: number
  wysokosc: number
...
`;
  ...
  const newText = `export type ProporcjaMiniaturyZadania = '16:9' | '4:3' | '1:1'
...
`;
  ...
  text = replaceOnce(text, oldText, newText, 'model miniatury');
  write(path, text);
}
...
// ---------------------------------------------------------------------------
{
  const path = 'src/moduly/zamkniete/pulpit/logika/miniaturyZadan.ts';
  const text = `import type { KadrMiniaturyZadania, MiniaturaZadaniaPulpitu, ProporcjaMiniaturyZadania } from '../modele/pulpit'
...
// ---------------------------------------------------------------------------
{
  const path = 'src/moduly/zamkniete/pulpit/uslugi/magazynPulpitu.ts';
  let text = read(path);
  ...
  const oldText = `function normalizujMiniatureZadania(wartosc: unknown): MiniaturaZadaniaPulpitu | null {
...
`;
  ...
  const newText = `function normalizujMiniatureZadania(wartosc: unknown): MiniaturaZadaniaPulpitu | null {
  if (!wartosc || typeof wartosc !== 'object') return null
  const dane = wartosc as Record<string, unknown>
  const daneUrl = tekst(dane.daneUrl)
...
`;
  ...
  text = replaceOnce(text, oldText, newText, 'normalizator miniatury');
  write(path, text);
}
...
// ---------------------------------------------------------------------------
{
  ...
  const oldMini = `function MiniaturaZadaniaNaOsi({ zadanie, zakresDniaPracy }: { zadanie: ZadaniePulpitu; zakresDniaPracy: ZakresDniaPracy }) {
...
`;
  ...
  const newMini = `function MiniaturaZadaniaNaOsi({ zadanie, zakresDniaPracy, otworz }: { zadanie: ZadaniePulpitu; zakresDniaPracy: ZakresDniaPracy; otworz: () => void }) {
  const godzinaMarkera = pobierzGodzineMarkeraZadania(zadanie, zakresDniaPracy)
  if (!zadanie.miniatura || !godzinaMarkera) return null
...
`;
  text = replaceOnce(text, oldMini, newMini, 'render miniatury osi');
...
`,
    'otwarcie edytora po imporcie',
  );

  const insertAnchor = `  function obsluzWklejenie(zdarzenie: ClipboardEvent<HTMLFormElement>) {
`;
  const insert = `  async function zmienKadrMiniatury(zmiana: Partial<KadrMiniaturyZadania>) {
...
`;
  if (!text.includes(insertAnchor)) throw new Error('Nie znaleziono miejsca wstawienia funkcji kadru.');
  text = text.replace(insertAnchor, insert + insertAnchor);
...
`;
  text = replaceOnce(text, oldUi, newUi, 'UI miniatury');
...
// ---------------------------------------------------------------------------
{
  const path = 'src/moduly/zamkniete/pulpit/pulpit.css';
  let text = read(path);
  const marker = '/* MINIATURY ZADAŃ NA OSI CZASU */';
  if (text.includes(marker)) throw new Error('CSS miniatur z tego patcha już istnieje — patch nie będzie nakładany drugi raz.');
...
`;
  write(path, text.trimEnd() + append + '\n');
}
...
// ---------------------------------------------------------------------------
{
  const path = 'testy/pulpit.regresja.test.ts';
  let text = read(path);
  ...
  const marker = "test('kadr miniatury ma bezpieczne wartości domyślne";
  if (text.includes(marker)) throw new Error('Testy miniatur z tego patcha już istnieją.');
...
`;
  ...
  write(path, text.trimEnd() + tests + '\n');
}

console.log('Patch kodu został nałożony.');
NODE
