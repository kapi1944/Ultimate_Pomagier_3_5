...
export function PanelEdycjiSwobodnychBlokow({ bloki, zaznaczonyBlokId, blokiSzablonu, trybEdycjiSzablonu, onZmienTrybEdycjiSzablonu, onZmienBloki, onDodajObraz, liczbaStron, szerokoscStronyMm = SZEROKOSC_STRONY_A4_MM, wysokoscStronyMm = WYSOKOSC_STRONY_A4_MM }: WlasciwosciPanelu) {
  ...
  return <section className="edytor-blokow__panel" aria-label="Edytor bloków dokumentu">
    ...
    {!blok ? <p>Kliknij blok na podglądzie, aby edytować jego właściwości.</p> : blok.pochodzenie === 'szablon' && !trybEdycjiSzablonu ? <p>To element szablonu. Włącz „Edytuj układ szablonu”, aby zmienić jego właściwości lub odblokować pozycję.</p> : <div className="edytor-blokow__wlasciwosci">
      ...
      <label><input checked={Boolean(blok.ramka)} type="checkbox" onChange={(zdarzenie) => zmienBlok((obecny) => ({ ...obecny, ramka: zdarzenie.target.checked ? { kolor: '#172033', szerokoscPx: 1, styl: 'ciagla' } : undefined }))} /> Ramka bloku</label>
      {blok.ramka && <div className="edytor-blokow__siatka-liczb"><label>Kolor ramki<input type="color" value={blok.ramka.kolor} onChange={(zdarzenie) => zmienBlok((obecny) => ({ ...obecny, ramka: obecny.ramka ? { ...obecny.ramka, kolor: zdarzenie.target.value } : undefined }))} /></label><label>Styl ramki<select value={blok.ramka.styl} onChange={(zdarzenie) => zmienBlok((obecny) => ({ ...obecny, ramka: obecny.ramka ? { ...obecny.ramka, styl: zdarzenie.target.value as 'ciagla' | 'kreskowana' | 'kropkowana' } : undefined }))}><option value="ciagla">Ciągła</option><option value="kreskowana">Kreskowana</option><option value="kropkowana">Kropkowana</option></select></label></div>}
      {blok.typ === 'tekst' ? <>
