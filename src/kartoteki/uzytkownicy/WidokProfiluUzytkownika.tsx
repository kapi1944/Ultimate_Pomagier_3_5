  ...
  return <main className="widok profil-uzytkownika">
    ...
    {czyEdycja ? <form className="profil-uzytkownika__formularz" onSubmit={zapisz}>
      <section><h2>Dane kontaktowe</h2><div className="profil-uzytkownika__siatka">
        ...
        {aktualnyFormularz.emaile.map((email, indeks) => <Pole etykieta={`E-mail ${indeks + 1}`} key={`email-${indeks}`}><div className="profil-uzytkownika__zlozone-pole"><input onChange={(zdarzenie) => zmienPole('emaile', aktualnyFormularz.emaile.map((wartosc, indeksEmaila) => indeksEmaila === indeks ? zdarzenie.target.value : wartosc))} type="email" value={email} />{aktualnyFormularz.emaile.length > 1 && <button onClick={() => zmienPole('emaile', aktualnyFormularz.emaile.filter((_, indeksEmaila) => indeksEmaila !== indeks))} type="button">Usuń</button>}</div></Pole>)}
        {aktualnyFormularz.telefony.map((telefon, indeks) => <Pole etykieta={`Telefon ${indeks + 1}`} key={`telefon-${indeks}`}><div className="profil-uzytkownika__zlozone-pole"><PoleTelefonu onChange={(nowyTelefon) => zmienPole('telefony', aktualnyFormularz.telefony.map((wartosc, indeksTelefonu) => indeksTelefonu === indeks ? nowyTelefon : wartosc))} telefon={telefon} wymagany />{aktualnyFormularz.telefony.length > 1 && <button onClick={() => zmienPole('telefony', aktualnyFormularz.telefony.filter((_, indeksTelefonu) => indeksTelefonu !== indeks))} type="button">Usuń</button>}</div></Pole>)}
      </div><button onClick={() => zmienPole('emaile', [...aktualnyFormularz.emaile, ''])} type="button">Dodaj e-mail</button><button onClick={() => zmienPole('telefony', [...aktualnyFormularz.telefony, { prefiks: '+48', numer: '', krajIso2: 'PL', numerE164: '' }])} type="button">Dodaj telefon</button></section>
      {!czyWlasciciel && <section><h2>Dane organizacyjne</h2><div className="profil-uzytkownika__siatka"><Pole etykieta="Login"><input onChange={(zdarzenie) => zmienPole('login', zdarzenie.target.value)} value={aktualnyFormularz.login} /></Pole><Pole etykieta="Rola">{przegladany.rola === 'ARCHITEKT' ? <output>Chroniona rola Kacpra Madeja</output> : <select onChange={(zdarzenie) => zmienPole('rola', zdarzenie.target.value as FormularzUzytkownika['rola'])} value={aktualnyFormularz.rola}>{roleUzytkownikow.filter((rola) => rola !== 'ARCHITEKT').map((rola) => <option key={rola} value={rola}>{etykietyRol[rola]}</option>)}</select>}</Pole><Pole etykieta="Organizacja"><select onChange={(zdarzenie) => zmienPole('organizacja', zdarzenie.target.value as FormularzUzytkownika['organizacja'])} value={aktualnyFormularz.organizacja}>{Object.entries(etykietyOrganizacji).map(([organizacja, etykieta]) => <option key={organizacja} value={organizacja}>{etykieta}</option>)}</select></Pole><Pole etykieta="Status"><select onChange={(zdarzenie) => zmienPole('status', zdarzenie.target.value as FormularzUzytkownika['status'])} value={aktualnyFormularz.status}>{['AKTYWNY', 'ZABLOKOWANY', 'NIEAKTYWNY'].map((status) => <option key={status} value={status}>{status}</option>)}</select></Pole><Pole etykieta="Kolor profilu"><input onChange={(zdarzenie) => zmienPole('kolorProfilu', zdarzenie.target.value)} pattern="#[0-9a-fA-F]{6}" value={aktualnyFormularz.kolorProfilu} /></Pole><Pole etykieta="Aliasy historyczne (po przecinku)"><input onChange={(zdarzenie) => zmienPole('aliasyHistoryczne', zdarzenie.target.value.split(',').map((alias) => alias.trim()).filter(Boolean))} value={aktualnyFormularz.aliasyHistoryczne.join(', ')} /></Pole><Pole etykieta="Przypisany klient"><input onChange={(zdarzenie) => zmienPole('przypisanyKlientId', zdarzenie.target.value || undefined)} value={aktualnyFormularz.przypisanyKlientId ?? ''} /></Pole><Pole etykieta="Przypisany trener"><input onChange={(zdarzenie) => zmienPole('przypisanyTrenerId', zdarzenie.target.value || undefined)} value={aktualnyFormularz.przypisanyTrenerId ?? ''} /></Pole></div><label><input checked={aktualnyFormularz.wymagaZmianyHasla} onChange={(zdarzenie) => zmienPole('wymagaZmianyHasla', zdarzenie.target.checked)} type="checkbox" /> Wymaga zmiany hasła</label><fieldset><legend>Odznaki</legend>{(Object.keys(metadaneOdznak) as TypOdznaki[]).map((odznaka) => <label key={odznaka}><input checked={aktualnyFormularz.odznaki.includes(odznaka)} onChange={(zdarzenie) => zmienPole('odznaki', zdarzenie.target.checked ? [...aktualnyFormularz.odznaki, odznaka] : aktualnyFormularz.odznaki.filter((wartosc) => wartosc !== odznaka))} type="checkbox" />{metadaneOdznak[odznaka].etykieta}</label>)}</fieldset></section>}
