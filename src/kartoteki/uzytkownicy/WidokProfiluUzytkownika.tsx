  ...
  return <main className="widok profil-uzytkownika">
    ...
    {czyEdycja ? <form className="profil-uzytkownika__formularz" onSubmit={zapisz}>
      <section><h2>Dane kontaktowe</h2><div className="profil-uzytkownika__siatka">
        ...
        {aktualnyFormularz.telefony.map((telefon, indeks) => <Pole etykieta={`Telefon ${indeks + 1}`} key={`telefon-${indeks}`}><div className="profil-uzytkownika__zlozone-pole"><div className="profil-uzytkownika__telefon"><span>+48</span><input onChange={(zdarzenie) => zmienPole('telefony', aktualnyFormularz.telefony.map((wartosc, indeksTelefonu) => indeksTelefonu === indeks ? { ...wartosc, numer: zdarzenie.target.value } : wartosc))} inputMode="numeric" value={telefon.numer} /></div>{aktualnyFormularz.telefony.length > 1 && <button onClick={() => zmienPole('telefony', aktualnyFormularz.telefony.filter((_, indeksTelefonu) => indeksTelefonu !== indeks))} type="button">Usuń</button>}</div></Pole>)}
