...
import { deserializujDaneAnkiety, etykietyPresetowAnkiety, etykietyTypowPytanAnkiety, nazwyOrganizatorowAnkiety, podzielAnkieteNaStrony, pobierzSkalePytania, serializujDaneAnkiety, utworzBlokiSzablonuAnkiety, utworzDaneAnkietyZKontekstu, utworzDomyslnaSkaleAnkiety, utworzDomyslneDaneAnkiety, zastosujPresetAnkiety, type DaneAnkiety, type PresetAnkiety, type PytanieAnkiety, type SekcjaAnkiety, type TypPytaniaAnkiety } from './modelAnkiety'
...
function przeniesPozycje<T>(pozycje: T[], indeks: number, przesuniecie: -1 | 1) {
  const cel = indeks + przesuniecie
  if (cel < 0 || cel >= pozycje.length) return pozycje
  const wynik = [...pozycje]
  ;[wynik[indeks], wynik[cel]] = [wynik[cel], wynik[indeks]]
  return wynik
}

function EdytorSkali({ pytanie, zmienPytanie }: { pytanie: PytanieAnkiety; zmienPytanie: (aktualizacja: (pytanie: PytanieAnkiety) => PytanieAnkiety) => void }) {
  const skala = pobierzSkalePytania(pytanie)
  return <div className="generator-ankiet__ustawienia-pytania"><label>Liczba stopni<select value={skala.liczbaStopni} onChange={(zdarzenie) => zmienPytanie((obecne) => ({ ...obecne, skala: utworzDomyslnaSkaleAnkiety(Number(zdarzenie.target.value)) }))}>{[2, 3, 4, 5, 6, 7, 8, 9, 10].map((liczba) => <option key={liczba} value={liczba}>{liczba}</option>)}</select></label><label>Opis skali<input value={skala.opisSkali ?? ''} onChange={(zdarzenie) => zmienPytanie((obecne) => ({ ...obecne, skala: { ...pobierzSkalePytania(obecne), opisSkali: zdarzenie.target.value } }))} /></label><label>Skrajny opis lewy<input value={skala.opisLewy ?? ''} onChange={(zdarzenie) => zmienPytanie((obecne) => ({ ...obecne, skala: { ...pobierzSkalePytania(obecne), opisLewy: zdarzenie.target.value } }))} /></label><label>Skrajny opis prawy<input value={skala.opisPrawy ?? ''} onChange={(zdarzenie) => zmienPytanie((obecne) => ({ ...obecne, skala: { ...pobierzSkalePytania(obecne), opisPrawy: zdarzenie.target.value } }))} /></label>{skala.etykiety.map((etykieta, indeks) => <label key={indeks}>Etykieta {indeks + 1}<input value={etykieta} onChange={(zdarzenie) => zmienPytanie((obecne) => { const obecnaSkala = pobierzSkalePytania(obecne); const etykiety = [...obecnaSkala.etykiety]; etykiety[indeks] = zdarzenie.target.value; return { ...obecne, skala: { ...obecnaSkala, etykiety } } })} /></label>)}</div>
}
...
function EdytorSekcjiAnkiety({ dane, zmienDane }: { dane: DaneAnkiety; zmienDane: ZmienDane }) {
  ...
  const zmienPytanie = (sekcjaId: string, pytanieId: string, aktualizacja: (pytanie: PytanieAnkiety) => PytanieAnkiety) => zmienSekcje(sekcjaId, (sekcja) => ({ ...sekcja, pytania: sekcja.pytania.map((pytanie) => pytanie.id === pytanieId ? aktualizacja(pytanie) : pytanie) }))
  ...
  function upuscPytanie(zdarzenie: DragEvent, sekcjaId: string, pytanieDoceloweId: string) {
    ...
    zmienSekcje(sekcjaId, (sekcja) => {
      const zrodlo = sekcja.pytania.findIndex((pytanie) => pytanie.id === przeciaganePytanie.pytanieId)
      const cel = sekcja.pytania.findIndex((pytanie) => pytanie.id === pytanieDoceloweId)
  ...
  return <div className="generator-ankiet__edytor-sekcji">{dane.sekcje.map((sekcja, indeksSekcji) => <article className="generator-ankiet__karta-sekcji" key={sekcja.id}>
    <header><label className="generator-ankiet__widocznosc-sekcji"><input checked={sekcja.widoczna} type="checkbox" onChange={(zdarzenie) => zmienSekcje(sekcja.id, (obecna) => ({ ...obecna, widoczna: zdarzenie.target.checked }))} /> Włączona</label><div className="generator-ankiet__kolejnosc-sekcji"><button aria-label={`Przenieś sekcję wyżej: ${sekcja.nazwa}`} disabled={indeksSekcji === 0} onClick={() => zmienDane((obecne) => ({ ...obecne, sekcje: przeniesPozycje(obecne.sekcje, indeksSekcji, -1) }))} type="button">↑</button><button aria-label={`Przenieś sekcję niżej: ${sekcja.nazwa}`} disabled={indeksSekcji === dane.sekcje.length - 1} onClick={() => zmienDane((obecne) => ({ ...obecne, sekcje: przeniesPozycje(obecne.sekcje, indeksSekcji, 1) }))} type="button">↓</button></div></header>
    <label>Nazwa sekcji<input value={sekcja.nazwa} onChange={(zdarzenie) => zmienSekcje(sekcja.id, (obecna) => ({ ...obecna, nazwa: zdarzenie.target.value }))} /></label><label>Opis opcjonalny<input value={sekcja.opis ?? ''} onChange={(zdarzenie) => zmienSekcje(sekcja.id, (obecna) => ({ ...obecna, opis: zdarzenie.target.value }))} /></label>
    <ol className="generator-ankiet__lista-pytan">{sekcja.pytania.map((pytanie, indeksPytania) => <li className="generator-ankiet__pytanie" draggable key={pytanie.id} onDragOver={(zdarzenie) => zdarzenie.preventDefault()} onDragStart={() => ustawPrzeciaganePytanie({ sekcjaId: sekcja.id, pytanieId: pytanie.id })} onDrop={(zdarzenie) => upuscPytanie(zdarzenie, sekcja.id, pytanie.id)}>
      <span aria-hidden="true" className="generator-ankiet__uchwyt-pytania">⋮⋮</span><div><input aria-label="Treść pytania" value={pytanie.tekst} onChange={(zdarzenie) => zmienPytanie(sekcja.id, pytanie.id, (obecne) => ({ ...obecne, tekst: zdarzenie.target.value }))} /><select aria-label="Typ pytania" value={pytanie.typ} onChange={(zdarzenie) => zmienPytanie(sekcja.id, pytanie.id, (obecne) => { const typ = zdarzenie.target.value as TypPytaniaAnkiety; return { ...obecne, typ, ...(typ === 'SKALA' ? { skala: pobierzSkalePytania(obecne) } : {}), ...(typ === 'JEDNOKROTNY_WYBOR' ? { opcje: obecne.opcje?.length ? obecne.opcje : ['Opcja 1', 'Opcja 2'] } : {}) } })}>{Object.entries(etykietyTypowPytanAnkiety).map(([wartosc, etykieta]) => <option key={wartosc} value={wartosc}>{etykieta}</option>)}</select><label className="generator-ankiet__wymagane"><input checked={pytanie.wymagane === true} type="checkbox" onChange={(zdarzenie) => zmienPytanie(sekcja.id, pytanie.id, (obecne) => ({ ...obecne, wymagane: zdarzenie.target.checked }))} /> Wymagane</label>{(pytanie.typ === 'SKALA' || pytanie.typ === 'OCENA_4') && <EdytorSkali pytanie={pytanie} zmienPytanie={(aktualizacja) => zmienPytanie(sekcja.id, pytanie.id, aktualizacja)} />}{pytanie.typ === 'JEDNOKROTNY_WYBOR' && <label>Opcje (jedna w wierszu)<textarea rows={3} value={(pytanie.opcje ?? []).join('\n')} onChange={(zdarzenie) => zmienPytanie(sekcja.id, pytanie.id, (obecne) => ({ ...obecne, opcje: zdarzenie.target.value.split(/\r?\n/).filter(Boolean) }))} /></label>}</div><div className="generator-ankiet__akcje-pytania"><button aria-label="Przenieś pytanie wyżej" disabled={indeksPytania === 0} onClick={() => zmienSekcje(sekcja.id, (obecna) => ({ ...obecna, pytania: przeniesPozycje(obecna.pytania, indeksPytania, -1) }))} type="button">↑</button><button aria-label="Przenieś pytanie niżej" disabled={indeksPytania === sekcja.pytania.length - 1} onClick={() => zmienSekcje(sekcja.id, (obecna) => ({ ...obecna, pytania: przeniesPozycje(obecna.pytania, indeksPytania, 1) }))} type="button">↓</button><button onClick={() => zmienSekcje(sekcja.id, (obecna) => ({ ...obecna, pytania: [...obecna.pytania, { ...pytanie, id: utworzId('pytanie'), tekst: `${pytanie.tekst} — kopia` }] }))} type="button">Duplikuj</button><button className="generator-ankiet__akcja-usun" onClick={() => zmienSekcje(sekcja.id, (obecna) => ({ ...obecna, pytania: obecna.pytania.filter((pozycja) => pozycja.id !== pytanie.id) }))} type="button">Usuń</button></div>
    </li>)}</ol><button className="generator-ankiet__przycisk-pomocniczy" onClick={() => zmienSekcje(sekcja.id, (obecna) => ({ ...obecna, pytania: [...obecna.pytania, { id: utworzId('pytanie'), typ: 'SKALA', tekst: 'Nowe pytanie', wymagane: true, skala: utworzDomyslnaSkaleAnkiety(5) }] }))} type="button">Dodaj pytanie</button>
  </article>)}<button className="generator-ankiet__przycisk-pomocniczy" onClick={() => zmienDane((obecne) => ({ ...obecne, sekcje: [...obecne.sekcje, { id: utworzId('sekcja'), nazwa: 'Nowa sekcja', widoczna: true, pytania: [] }] }))} type="button">Dodaj sekcję</button></div>
}
...
function FormularzAnkiety({ dane, grupy, prefiksId, szczegoly, szczegolyId, ustawDane, ustawGrupeId, ustawSzczegolyId, grupaId }: { dane: DaneAnkiety; grupy: { id: string; nazwa: string }[]; prefiksId: string; szczegoly: SzczegolyDoGeneratoraDokumentu[]; szczegolyId: string; ustawDane: Dispatch<SetStateAction<DaneAnkiety>>; ustawGrupeId: (id: string) => void; ustawSzczegolyId: (id: string) => void; grupaId: string }) {
  ...
  return <div className="generator-ankiet__formularz"><SekcjaFormularza domyslnieOtwarta tytul="Dane szkolenia"><div className="generator-ankiet__siatka-pol"><label className="generator-ankiet__pole-szerokie" htmlFor={`${prefiksId}-szczegoly`}>Szczegóły organizacyjne<select id={`${prefiksId}-szczegoly`} value={szczegolyId} onChange={(zdarzenie) => ustawSzczegolyId(zdarzenie.target.value)}><option value="">Uzupełnij ręcznie</option>{szczegoly.map((pozycja) => <option key={pozycja.id} value={pozycja.id}>{pozycja.nazwa}{pozycja.czyKopiaRobocza ? ' — kopia robocza' : ''}</option>)}</select></label>{szczegolyId && <label className="generator-ankiet__pole-szerokie" htmlFor={`${prefiksId}-grupa`}>Grupa szkoleniowa<select id={`${prefiksId}-grupa`} value={grupaId} onChange={(zdarzenie) => ustawGrupeId(zdarzenie.target.value)}>{grupy.map((grupa) => <option key={grupa.id} value={grupa.id}>{grupa.nazwa}</option>)}</select></label>}<label className="generator-ankiet__pole-szerokie" htmlFor={`${prefiksId}-tytul`}>Tytuł szkolenia<input id={`${prefiksId}-tytul`} value={dane.tytulSzkolenia} onChange={(zdarzenie) => zmienDane((obecne) => ({ ...obecne, tytulSzkolenia: zdarzenie.target.value }))} /></label><label htmlFor={`${prefiksId}-data-od`}>Data od<input id={`${prefiksId}-data-od`} type="date" value={dane.dataOd} onChange={(zdarzenie) => zmienDane((obecne) => ({ ...obecne, dataOd: zdarzenie.target.value }))} /></label><label htmlFor={`${prefiksId}-data-do`}>Data do<input id={`${prefiksId}-data-do`} type="date" value={dane.dataDo} onChange={(zdarzenie) => zmienDane((obecne) => ({ ...obecne, dataDo: zdarzenie.target.value }))} /></label><label className="generator-ankiet__pole-szerokie" htmlFor={`${prefiksId}-miejsce`}>Miejsce / tryb<input id={`${prefiksId}-miejsce`} value={dane.miejsce} onChange={(zdarzenie) => zmienDane((obecne) => ({ ...obecne, miejsce: zdarzenie.target.value }))} /></label><label className="generator-ankiet__pole-szerokie" htmlFor={`${prefiksId}-trener`}>Trener / ekspert<input id={`${prefiksId}-trener`} value={dane.trener} onChange={(zdarzenie) => zmienDane((obecne) => ({ ...obecne, trener: zdarzenie.target.value }))} /></label></div><p className="generator-ankiet__opis-wygladu">Zmiany dotyczą wyłącznie tej ankiety; szkolenie pozostaje niezmienione.</p></SekcjaFormularza><SekcjaFormularza domyslnieOtwarta tytul="Szablon"><label htmlFor={`${prefiksId}-preset`}>Wariant<select id={`${prefiksId}-preset`} value={dane.preset} onChange={(zdarzenie) => ustawDane((obecne) => zastosujPresetAnkiety(obecne, zdarzenie.target.value as PresetAnkiety))}>{Object.entries(etykietyPresetowAnkiety).map(([wartosc, etykieta]) => <option key={wartosc} value={wartosc}>{etykieta}</option>)}</select></label><label htmlFor={`${prefiksId}-organizator`}>Organizator<select id={`${prefiksId}-organizator`} value={dane.organizator} onChange={(zdarzenie) => zmienDane((obecne) => ({ ...obecne, preset: 'WLASNA', organizator: zdarzenie.target.value === 'IIST' ? 'IIST' : 'SEMPER' }))}>{Object.entries(nazwyOrganizatorowAnkiety).map(([wartosc, etykieta]) => <option key={wartosc} value={wartosc}>{etykieta}</option>)}</select></label><p className="generator-ankiet__aktywny-preset">Bazowe warianty „Oryginalny” są chronione. Wybranie nowego wariantu z własnej ankiety zachowuje treść pytań.</p></SekcjaFormularza><SekcjaFormularza domyslnieOtwarta tytul="Sekcje i pytania">{dane.preset === 'WLASNA' ? <EdytorSekcjiAnkiety dane={dane} zmienDane={zmienDane} /> : <div className="generator-ankiet__ochrona-presets"><p>Treść bazowego wariantu jest chroniona przed przypadkową zmianą.</p><button className="generator-ankiet__przycisk-pomocniczy" onClick={() => ustawDane((obecne) => zastosujPresetAnkiety(obecne, 'WLASNA'))} type="button">Utwórz własną kopię do edycji</button></div>}</SekcjaFormularza><SekcjaFormularza tytul="Układ dokumentu"><p className="generator-ankiet__opis-wygladu">Logo, nagłówki i dodatkowe teksty edytujesz w panelu układu. Tabela skali i pytania pozostają kontrolowane przez renderer.</p></SekcjaFormularza><SekcjaFormularza tytul="Eksport"><p className="generator-ankiet__opis-wygladu">Podgląd, druk i PDF korzystają z tego samego układu stron A4.</p></SekcjaFormularza></div>
}
