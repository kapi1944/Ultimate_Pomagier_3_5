...
function DrukChecklisty({ dane, nazwaOpiekuna, nazwaWysylacza }: { dane: DaneChecklistyPaczki; nazwaOpiekuna: string; nazwaWysylacza: string }) {
  ...
  return <section aria-label="Podgląd wydruku Checklisty paczki" className="checklista-paczki__wydruk" id="wydruk-checklisty">
    ...
    <section className="checklista-paczki__wydruk-dane"><p><strong>Liczba osób:</strong> {uczestnicy}</p><p><strong>Klient:</strong> {dane.klient || 'brak'}</p><p><strong>Grupa:</strong> {migawka?.nazwaGrupy || 'brak'}</p><p><strong>Trener / trenerzy:</strong> {migawka?.trenerzy.join(', ') || 'brak'}</p><p><strong>Data i miejsce:</strong> {formatujTerminyZPrzerwami(migawka?.terminy ?? []) || 'brak'} · {migawka?.miejsce || 'brak'}</p><p><strong>Opiekun:</strong> {nazwaOpiekuna}</p></section>
    ...
    <section className="checklista-paczki__wydruk-wysylka"><h3>Wysyłka paczki</h3><p><strong>Adres do wysyłki paczki:</strong> {formatujAdres(dane.daneOdbiorcy) || 'brak'}</p><p><strong>Odbiorca paczki:</strong> {dane.daneOdbiorcy.imieNazwisko || dane.daneOdbiorcy.nazwaFirmy || 'brak'}</p><p><strong>Waga paczki:</strong> {dane.waga || '—'} · <strong>Wysokość paczki:</strong> {dane.wysokosc || '—'} · <strong>Data wysłania:</strong> {dane.dataWyslania || '—'}</p><p><strong>Wysyłacz / pakujący:</strong> {dane.osobaPakujaca || nazwaWysylacza || '_______________________________'}</p><p><strong>Podpis pakującego:</strong> _________________________________________________</p></section>
  </section>
}
...
export default function WidokChecklistPaczek({ dokumentIdZTrasy }: WlasciwosciWidokuChecklistPaczek) {
  ...
  if (!dokument) {
    ...
    return <section className="widok checklista-paczki checklista-paczki--start"><header><h1>Checklisty paczek</h1><p>Przygotuj roboczą Checklistę dla konkretnej grupy szkoleniowej.</p>{komunikat && <p aria-live="polite" className="checklista-paczki__komunikat">{komunikat}</p>}</header><section className="checklista-paczki__karta"><h2>Nowa checklista paczki</h2><ol className="checklista-paczki__kroki"><li><label><strong>Krok 1. Szczegóły organizacyjne</strong><select value={wybraneSzczegolyId} onChange={(zdarzenie) => { ustawWybraneSzczegolyId(zdarzenie.target.value); ustawWybranaGrupeId('') }}><option value="">Wybierz Szczegóły organizacyjne</option>{szczegoly.map((pozycja) => <option key={pozycja.id} value={pozycja.id}>{pozycja.nazwa}</option>)}</select></label></li><li><label><strong>Krok 2. Grupa szkoleniowa</strong><select disabled={!wybraneSzczegoly} value={wybranaGrupaId} onChange={(zdarzenie) => ustawWybranaGrupeId(zdarzenie.target.value)}><option value="">Wybierz grupę</option>{wybraneSzczegoly?.grupy.map((grupa) => <option key={grupa.id} value={grupa.id}>{grupa.nazwa} · {grupa.liczbaUczestnikow} uczestników</option>)}</select></label></li><li><strong>Krok 3. Utwórz Checklistę</strong>{liczbaDlaWybranejGrupy > 0 && <p>Dla tej grupy istnieje już {liczbaDlaWybranejGrupy} {liczbaDlaWybranejGrupy === 1 ? 'Checklista paczki' : 'Checklisty paczek'}.</p>}<button disabled={!wybraneSzczegolyId || !wybranaGrupaId} type="button" onClick={utworzZeSzczegolow}>{liczbaDlaWybranejGrupy ? 'Utwórz kolejną Checklistę' : 'Utwórz Checklistę paczki'}</button></li></ol></section><section className="checklista-paczki__karta"><h2>Istniejące checklisty</h2>{!checklisty.length && <p>Brak checklist.</p>}<div className="checklista-paczki__lista">{checklisty.map((pozycja) => { const migawka = pozycja.daneDokumentu.migawkaZrodla; return <article key={pozycja.id}><div><strong>{pozycja.daneDokumentu.identyfikator}</strong><span>{migawka?.tytulSzkolenia || 'Brak szkolenia'} · {migawka?.nazwaGrupy || 'Brak grupy'}</span><span>{formatujTerminyZPrzerwami(migawka?.terminy ?? []) || 'brak terminu'} · {pozycja.daneDokumentu.statusChecklisty}</span></div><div><span>Skan: {pozycja.daneDokumentu.zalaczniki.some((zalacznik) => zalacznik.typ === 'SKAN_PODPISANEJ_CHECKLISTY') ? 'dołączony' : 'brak'}</span>{pozycja.daneDokumentu.pilna && <strong className="checklista-paczki__pilna">Pilna</strong>}<button type="button" onClick={() => otworzCheckliste(pozycja.id)}>Otwórz</button></div></article> })}</div></section></section>
  }
